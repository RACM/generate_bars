...
-filter_complex "[1:a][2:a]join=inputs=2[a0]" -map 0:v -map "[a0]" -c:v ${LibEnc} -g 60 -b:v ${MV}M -minrate ${MV}M -maxrate ${MV}M -muxrate ${MXO}M  -preset veryfast -pix_fmt yuv420p \
-f mpegts "udp://${IPO}:${PO}?pkt_size=1316" > /dev/null 2>&1 < /dev/null &
    elif [[ $dvp -eq 0 ]]; then
...
ffmpeg_stop () {
    ...
    if [[ -z $PID ]]; then
        ...
        echo " "
	    echo "FFMPEG Stream on ${IPO}:${PO} Stopped"
        echo " "
        exit 0
    fi
}
...
deleteStream () {
    echo " "
	echo "What is the exisiting stream IP Output: "
    read IPO
    echo "What is the existing stream PORT Output: "
    read PO
    echo " "
...
listStreams () {
    nst=`ps -ef | grep -v grep | grep ffmpeg | awk '{print $53}' | cut -d\? -f1 | wc -l | awk '{print $1}'`
    nst=$(echo ${nst} | awk '{print $1*1}')
    strms=(`ps -ef | grep -v grep | grep ffmpeg | awk '{print $53}' | cut -d\? -f1`)
